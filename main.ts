...
export default class RememberCursorPosition extends Plugin {
  ...
  async onload() {
    ...
    const cursorTracker = ViewPlugin.fromClass(class {
      ...
      update(update: ViewUpdate) {
        ...
        const currentState = {
          cursor: {
            from: {
              line: fromPos.number - 1, // CodeMirror is 1-based, convert to 0-based
              ch: selection.from - fromPos.from // Get column position
            },
            to: {
              line: toPos.number - 1,
	...
	private saveFilePositionIfChanged(fileName: string, currentState: EphemeralState): void {
    ...
    const previousState = this.db[fileName];
		...
		if (!previousState || (!isNaN(currentState.scroll) && !this.isEphemeralStatesEquals(currentState, previousState))) {
    ...
    const loadedLeafId = [activeLeaf.id, activeLeaf.getViewState().state.file] as [LeafId, FileName];
	...
	private async saveEphemeralState(st: EphemeralState) {
		...
		if (fileName) { //do not save if file changed or was not loaded
			this.db[fileName] = st;
		}
	}
